<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-6">

    <!-- Search bar with autocomplete -->
    <div class="relative mb-3">
      <Search class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 z-10" />
      <input
        ref="searchInputRef"
        v-model="searchQuery"
        type="search"
        autocomplete="off"
        autocorrect="off"
        autocapitalize="off"
        spellcheck="false"
        :placeholder="t('marketPlayers.searchPlaceholder')"
        class="w-full pl-10 pr-4 py-2.5 text-sm md:text-base rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-500 transition-colors"
        @focus="showSuggestions = true"
        @blur="onBlur"
        @input="onInputNative"
        @keyup="onInputNative"
        @keydown.escape="showSuggestions = false"
        @keydown.down.prevent="navigateSuggestion(1)"
        @keydown.up.prevent="navigateSuggestion(-1)"
        @keydown.enter.prevent="selectHighlighted"
      />

      <!-- Autocomplete dropdown -->
      <div
        v-if="showSuggestions && (teamSuggestions.length > 0 || playerSuggestions.length > 0)"
        class="absolute left-0 right-0 top-full mt-1 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg z-50 max-h-80 overflow-y-auto"
      >
        <!-- Team suggestions -->
        <div v-if="teamSuggestions.length > 0">
          <div class="px-3 py-1.5 text-[10px] font-semibold uppercase text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-800/50">
            {{ t('marketPlayers.searchTeams') }}
          </div>
          <button
            v-for="(team, idx) in teamSuggestions"
            :key="`team-${team.id}`"
            class="w-full flex items-center gap-2.5 px-3 py-2 text-left hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
            :class="{ 'bg-cyan-50 dark:bg-cyan-900/20': highlightedIndex === idx }"
            @mousedown.prevent="selectTeam(team)"
          >
            <img :src="team.logo" :alt="team.name" class="w-5 h-5 object-contain" />
            <span class="text-sm text-gray-900 dark:text-white">{{ team.name }}</span>
            <span class="text-xs text-gray-400">({{ team.short_name }})</span>
          </button>
        </div>

        <!-- Player suggestions -->
        <div v-if="playerSuggestions.length > 0">
          <div class="px-3 py-1.5 text-[10px] font-semibold uppercase text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-800/50">
            {{ t('marketPlayers.searchPlayers') }}
          </div>
          <button
            v-for="(player, idx) in playerSuggestions"
            :key="`player-${player.id}`"
            class="w-full flex items-center gap-2.5 px-3 py-2 text-left hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
            :class="{ 'bg-cyan-50 dark:bg-cyan-900/20': highlightedIndex === teamSuggestions.length + idx }"
            @mousedown.prevent="selectPlayer(player)"
          >
            <img :src="player.profile_image" :alt="player.name" class="w-6 h-6 rounded-full object-cover bg-gray-200 dark:bg-gray-700" />
            <span class="text-sm text-gray-900 dark:text-white">{{ player.name }}</span>
            <span class="text-xs text-gray-400">{{ player.team_short_name }}</span>
          </button>
        </div>
      </div>
    </div>

    <!-- Selected search chips -->
    <div v-if="selectedFilters.length > 0" class="flex flex-wrap gap-1.5 mb-3">
      <span
        v-for="sel in selectedFilters"
        :key="`${sel.type}-${sel.id}`"
        class="inline-flex items-center gap-1 pl-2 pr-1 py-1 text-xs font-medium rounded-full bg-cyan-100 dark:bg-cyan-900/40 text-cyan-700 dark:text-cyan-300"
      >
        <img
          v-if="sel.type === 'team'"
          :src="getTeamLogo(sel.id)"
          class="w-3.5 h-3.5 object-contain"
        />
        {{ sel.label }}
        <button
          class="ml-0.5 p-0.5 rounded-full hover:bg-cyan-200 dark:hover:bg-cyan-800 transition-colors"
          @click="removeFilter(sel)"
        >
          <X class="w-3 h-3" />
        </button>
      </span>
      <button
        class="text-[10px] text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 px-2"
        @click="clearAllFilters"
      >
        {{ t('marketPlayers.clearAll') }}
      </button>
    </div>

    <!-- Position filter chips (multi-select) -->
    <div class="flex gap-2 mb-2">
      <button
        v-for="pos in positionFilters"
        :key="pos.value"
        class="flex-shrink-0 px-3 py-1.5 text-xs font-medium rounded-full transition-all"
        :class="[
          selectedPositions.includes(pos.value)
            ? `${pos.activeBg} text-white shadow-sm`
            : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'
        ]"
        @click="togglePosition(pos.value)"
      >
        {{ pos.label }}
      </button>
    </div>

    <!-- Preset filter chips (single-select) -->
    <div
      class="flex gap-2 overflow-x-auto scrollbar-none pb-3 -mx-4 px-4 sm:mx-0 sm:px-0"
    >
      <button
        v-for="chip in presetChips"
        :key="chip.key"
        class="flex-shrink-0 px-3 py-1.5 text-xs font-medium rounded-full transition-all"
        :class="[
          activePreset === chip.key
            ? 'bg-cyan-500 text-white shadow-sm'
            : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'
        ]"
        @click="setPreset(chip.key)"
      >
        <span v-if="chip.icon" class="mr-1">{{ chip.icon }}</span>{{ chip.label }}
      </button>
    </div>

    <!-- Toolbar: sort + count + view toggle -->
    <div class="flex items-center justify-between gap-3 mb-3">
      <!-- Sort dropdown -->
      <div class="flex items-center gap-2">
        <ArrowUpDown class="w-3.5 h-3.5 text-gray-400" />
        <select
          v-model="activeSort"
          class="text-xs font-medium bg-transparent text-gray-600 dark:text-gray-400 border-none focus:outline-none focus:ring-0 cursor-pointer appearance-none pr-4"
          :style="{ backgroundImage: 'none' }"
        >
          <option v-for="opt in sortOptions" :key="opt.key" :value="opt.key">
            {{ opt.label }}
          </option>
        </select>
      </div>

      <!-- Results count + view toggle -->
      <div class="flex items-center gap-3">
        <span class="text-xs text-gray-400 dark:text-gray-500">
          {{ filteredPlayers.length }} {{ t('marketPlayers.players') }}
        </span>
        <div class="flex bg-gray-100 dark:bg-gray-800 rounded-lg p-0.5">
          <button
            class="p-1.5 rounded-md transition-colors"
            :class="viewMode === 'list'
              ? 'bg-white dark:bg-gray-700 shadow-sm text-gray-900 dark:text-white'
              : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'"
            @click="setViewMode('list')"
            :title="t('marketPlayers.listView')"
          >
            <List class="w-4 h-4" />
          </button>
          <button
            class="p-1.5 rounded-md transition-colors"
            :class="viewMode === 'card'
              ? 'bg-white dark:bg-gray-700 shadow-sm text-gray-900 dark:text-white'
              : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'"
            @click="setViewMode('card')"
            :title="t('marketPlayers.cardView')"
          >
            <LayoutGrid class="w-4 h-4" />
          </button>
        </div>
      </div>
    </div>

    <!-- Player list (row view) -->
    <div v-if="viewMode === 'list'" class="flex flex-col gap-2">
      <PlayerRow
        v-for="player in filteredPlayers"
        :key="player.id"
        :player="player"
        @click="openPlayer(player)"
      />
    </div>

    <!-- Player grid (card view) -->
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
      <PlayerCard
        v-for="player in filteredPlayers"
        :key="player.id"
        :player="player"
        @click="openPlayer(player)"
      />
    </div>

    <!-- Empty state -->
    <div
      v-if="filteredPlayers.length === 0"
      class="rounded-xl bg-gray-50 dark:bg-gray-800/50 p-8 text-center mt-4"
    >
      <p class="text-gray-500 dark:text-gray-400">{{ t('marketPlayers.noResults') }}</p>
    </div>

    <!-- Player modal -->
    <PlayerModal
      v-if="selectedPlayerId"
      :player-id="selectedPlayerId"
      :open="!!selectedPlayerId"
      @close="selectedPlayerId = null"
    />
  </div>
</template>

<script setup lang="ts">
import { ref, computed, watch } from 'vue'
import { useI18n } from 'vue-i18n'
import { onBeforeRouteLeave } from 'vue-router'
import { Search, ArrowUpDown, List, LayoutGrid, X } from 'lucide-vue-next'
import PlayerRow from '@/components/market/PlayerRow.vue'
import PlayerCard from '@/components/market/PlayerCard.vue'
import PlayerModal from '@/components/player/PlayerModal.vue'
import { mockMarketPlayers, mockTeams } from '@/mocks/marketMock'
import type { MarketPlayer, MarketPresetFilter, MarketSortOption, MarketViewMode, SearchSelection } from '@/types/market'
import { POSITION_COLORS } from '@/types/player'

const { t } = useI18n()

// ── Navigation guard: close modal on back button instead of leaving ──
onBeforeRouteLeave((_to, _from, next) => {
  if (selectedPlayerId.value) {
    // Modal is open - close it instead of navigating
    selectedPlayerId.value = null
    next(false) // Cancel navigation
  } else {
    next() // Allow navigation
  }
})

// ── State ──────────────────────────────────────
const searchQuery = ref('')
const searchInputRef = ref<HTMLInputElement | null>(null)
const showSuggestions = ref(false)
const highlightedIndex = ref(-1)
const selectedFilters = ref<SearchSelection[]>([])
const selectedPositions = ref<number[]>([])
const activePreset = ref<MarketPresetFilter>('all')
const activeSort = ref<MarketSortOption>('market_value')
const viewMode = ref<MarketViewMode>(
  (localStorage.getItem('marketViewMode') as MarketViewMode) || 'list'
)
const selectedPlayerId = ref<string | null>(null)

// ── Position filter chips ──────────────────────
const positionFilters = computed(() => [
  { value: 1, label: t('position.gk'), activeBg: POSITION_COLORS[1].bg },
  { value: 2, label: t('position.def'), activeBg: POSITION_COLORS[2].bg },
  { value: 3, label: t('position.mid'), activeBg: POSITION_COLORS[3].bg },
  { value: 4, label: t('position.fwd'), activeBg: POSITION_COLORS[4].bg },
])

// ── Preset filter chips ────────────────────────
const presetChips = computed(() => [
  { key: 'all' as const, label: t('marketPlayers.chipAll'), icon: null },
  { key: 'hot' as const, label: t('marketPlayers.chipHot'), icon: '🔥' },
  { key: 'falling' as const, label: t('marketPlayers.chipFalling'), icon: '📉' },
  { key: 'top_ppm' as const, label: t('marketPlayers.chipTopPPM'), icon: '⭐' },
  { key: 'top_points' as const, label: t('marketPlayers.chipTopPoints'), icon: '🏆' },
])

// ── Sort options (all 9) ───────────────────────
const sortOptions = computed(() => [
  { key: 'market_value' as const, label: t('marketPlayers.sortValue') },
  { key: 'total_points' as const, label: t('marketPlayers.sortTotalPoints') },
  { key: 'avg_points' as const, label: t('marketPlayers.sortAvgPoints') },
  { key: 'ppm' as const, label: t('marketPlayers.sortPPM') },
  { key: 'value_change_7d' as const, label: t('marketPlayers.sortValueChange') },
  { key: 'form' as const, label: t('marketPlayers.sortForm') },
  { key: 'stability' as const, label: t('marketPlayers.sortStability') },
  { key: 'points_per_minute' as const, label: t('marketPlayers.sortPointsPerMinute') },
  { key: 'raw_points_avg' as const, label: t('marketPlayers.sortRawPoints') },
])

// ── Search suggestions ─────────────────────────
const teamSuggestions = computed(() => {
  if (!searchQuery.value.trim()) return []
  const q = searchQuery.value.toLowerCase().trim()
  const selectedTeamIds = selectedFilters.value.filter(f => f.type === 'team').map(f => f.id)
  return mockTeams
    .filter(team =>
      !selectedTeamIds.includes(team.id) &&
      (team.name.toLowerCase().includes(q) || team.short_name.toLowerCase().includes(q))
    )
    .slice(0, 5)
})

const playerSuggestions = computed(() => {
  if (!searchQuery.value.trim()) return []
  const q = searchQuery.value.toLowerCase().trim()
  const selectedPlayerIds = selectedFilters.value.filter(f => f.type === 'player').map(f => f.id)
  return mockMarketPlayers
    .filter(player =>
      !selectedPlayerIds.includes(player.id) &&
      player.name.toLowerCase().includes(q)
    )
    .slice(0, 5)
})

// Reset highlight when suggestions change
watch([teamSuggestions, playerSuggestions], () => {
  highlightedIndex.value = -1
})

// Show suggestions when search query changes (more reliable on mobile than @input)
watch(searchQuery, (newVal) => {
  if (newVal.trim()) {
    showSuggestions.value = true
  }
})

// ── Derived: filtered + sorted players ─────────
const filteredPlayers = computed(() => {
  let players = [...mockMarketPlayers]

  // Filter by selected teams
  const selectedTeamIds = selectedFilters.value.filter(f => f.type === 'team').map(f => f.id)
  if (selectedTeamIds.length > 0) {
    players = players.filter(p => selectedTeamIds.includes(p.team_id))
  }

  // Filter by selected players (show only those specific players)
  const selectedPlayerIds = selectedFilters.value.filter(f => f.type === 'player').map(f => f.id)
  if (selectedPlayerIds.length > 0) {
    players = players.filter(p => selectedPlayerIds.includes(p.id))
  }

  // Position filter (multi-select)
  if (selectedPositions.value.length > 0) {
    players = players.filter(p => selectedPositions.value.includes(p.position))
  }

  // Preset filter
  switch (activePreset.value) {
    case 'hot':
      players = players.filter(p => p.value_change_7d > 0)
      break
    case 'falling':
      players = players.filter(p => p.value_change_7d < 0)
      break
    case 'top_ppm':
    case 'top_points':
      // No filtering, just sort override below
      break
  }

  // Determine sort key (preset might override)
  let sortKey = activeSort.value
  if (activePreset.value === 'hot') sortKey = 'value_change_7d'
  if (activePreset.value === 'falling') sortKey = 'value_change_7d'
  if (activePreset.value === 'top_ppm') sortKey = 'ppm'
  if (activePreset.value === 'top_points') sortKey = 'total_points'

  // Sort (stability: lower is better, so ascending; rest: descending)
  players.sort((a, b) => {
    const aVal = a[sortKey] as number
    const bVal = b[sortKey] as number
    // For "falling" chip, sort ascending (most negative first)
    if (activePreset.value === 'falling' && sortKey === 'value_change_7d') {
      return aVal - bVal
    }
    // For stability, lower is better
    if (sortKey === 'stability') {
      return aVal - bVal
    }
    return bVal - aVal
  })

  return players
})

// ── Actions ────────────────────────────────────
function togglePosition(pos: number) {
  const idx = selectedPositions.value.indexOf(pos)
  if (idx >= 0) {
    selectedPositions.value.splice(idx, 1)
  } else {
    selectedPositions.value.push(pos)
  }
}

function setPreset(key: MarketPresetFilter) {
  activePreset.value = key
}

function setViewMode(mode: MarketViewMode) {
  viewMode.value = mode
  localStorage.setItem('marketViewMode', mode)
}

function openPlayer(player: MarketPlayer) {
  selectedPlayerId.value = player.id
}

// ── Search autocomplete actions ────────────────
function onBlur() {
  // Delay to allow click on suggestion (longer delay for mobile)
  setTimeout(() => {
    showSuggestions.value = false
    highlightedIndex.value = -1
  }, 300)
}

function onInputNative(event: Event) {
  // Read directly from the input element for better mobile support
  const target = event.target as HTMLInputElement
  const value = target.value
  // Update the ref manually to ensure sync
  searchQuery.value = value
  // Show suggestions if there's text
  if (value.trim()) {
    showSuggestions.value = true
  }
}

function navigateSuggestion(dir: number) {
  const total = teamSuggestions.value.length + playerSuggestions.value.length
  if (total === 0) return
  highlightedIndex.value = (highlightedIndex.value + dir + total) % total
}

function selectHighlighted() {
  if (highlightedIndex.value < 0) return
  if (highlightedIndex.value < teamSuggestions.value.length) {
    selectTeam(teamSuggestions.value[highlightedIndex.value])
  } else {
    const playerIdx = highlightedIndex.value - teamSuggestions.value.length
    selectPlayer(playerSuggestions.value[playerIdx])
  }
}

function selectTeam(team: typeof mockTeams[0]) {
  selectedFilters.value.push({
    type: 'team',
    id: team.id,
    label: team.name,
  })
  searchQuery.value = ''
  showSuggestions.value = false
  highlightedIndex.value = -1
}

function selectPlayer(player: MarketPlayer) {
  selectedFilters.value.push({
    type: 'player',
    id: player.id,
    label: player.name,
  })
  searchQuery.value = ''
  showSuggestions.value = false
  highlightedIndex.value = -1
}

function removeFilter(sel: SearchSelection) {
  const idx = selectedFilters.value.findIndex(f => f.type === sel.type && f.id === sel.id)
  if (idx >= 0) {
    selectedFilters.value.splice(idx, 1)
  }
}

function clearAllFilters() {
  selectedFilters.value = []
}

function getTeamLogo(teamId: string): string {
  const team = mockTeams.find(t => t.id === teamId)
  return team?.logo || ''
}
</script>

<style scoped>
.scrollbar-none {
  -ms-overflow-style: none;
  scrollbar-width: none;
}
.scrollbar-none::-webkit-scrollbar {
  display: none;
}
</style>
